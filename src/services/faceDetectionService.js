            ...
            const encodings = await Promise.all(
                chunk.map(async (photo) => {
                    ...
                    return {
                        ...
                        encodings: encoding?.encoding, // Add encodings to the photo object
                    };
                })
            );
    ...
    try {
        ...
        for (const photoWithEncoding of photoWithEncodings) {
            ...
            for (let i=0; i< encodings?.length; i++) {
                ...
                const isMatchFound = existingEncodings?.length > 0 ? await matchFace(encodings[i], existingEncodings) : false;

                if(isMatchFound?.personId){
                    await photo.updateOne({ _id: photoWithEncoding?._id }, { $set: { $set: { clusterIds: { $push: isMatchFound?.personId }}}})
                }
                else {
